 ...
 * The MIT License (MIT)
 *
 * Copyright (c) 2014-2015 gjson@gjson.com
 *防支付宝 声波支付
 * 
 */
...
public class RippleLayout extends RelativeLayout {
	...
	private void initTypedArray(Context context, AttributeSet attrs) {
		final TypedArray typedArray = context.obtainStyledAttributes(attrs,
				R.styleable.RippleStyle);
		//
		mRippleColor = typedArray.getColor(R.styleable.RippleStyle_color,
				DEFAULT_RIPPLE_COLOR);
		mStrokeWidth = typedArray.getDimension(
				R.styleable.RippleStyle_strokewidth, DEFAULT_STROKE_WIDTH);
		mRippleRadius = typedArray.getDimension(
				R.styleable.RippleStyle_radiuss, DEFAULT_RADIUS);
		mAnimDuration = typedArray.getInt(R.styleable.RippleStyle_duration,
				DEFAULT_DURATION_TIME);
		mRippleViewNums = typedArray.getInt(R.styleable.RippleStyle_rippleNums,
				DEFAULT_RIPPLE_COUNT);
		mRippleScale = typedArray.getFloat(R.styleable.RippleStyle_scale,
				DEFAULT_SCALE);
